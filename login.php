...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password) || empty($verify_code_input)) {
        ...
        // A. Ambil user berdasarkan email (Menggunakan Prepared Statement)
        ...
        if ($user) {
            ...
            // B. Verifikasi Password (Wajib)
            if (password_verify($password, $user['password'])) {

                // C. Verifikasi Verify Code (Koreksi variabel!)
                if ($user['verify_code'] != $verify_code_input) { // MENGGUNAKAN $verify_code_input
                    $error = "Verify code salah!";
                } else {
                    
                    // --- LOGIN BERHASIL DAN CODE COCOK ---
                    ...
                    // 2. Cek pembayaran (tanpa Prepared Statement, tapi karena $id_user sudah pasti integer, ini aman)
                    $cek_bayar = mysqli_query($conn, "SELECT id_paket FROM pembayaran WHERE id_user = '$id_user'");
                    $pembayaran = mysqli_fetch_assoc($cek_bayar);

                    // 3. Simpan id paket di session
                    if ($pembayaran) {
