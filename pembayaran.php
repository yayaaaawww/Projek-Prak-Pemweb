...
// ----------------------------------------------------
// 1. AKSES KONTROL: PASTIKAN PENGGUNA SUDAH LOGIN
// ----------------------------------------------------

// Jika TIDAK ADA sesi 'user_id', arahkan ke login dengan parameter redirect
if (!isset($_SESSION['user_id']) || empty($_SESSION['user_id'])) {
    // Simpan parameter id_paket untuk redirect setelah login
    $redirect_url = "pembayaran.php";
    if (isset($_GET['id_paket'])) {
        $redirect_url .= "?id_paket=" . intval($_GET['id_paket']);
    }
    header("Location: login.php?redirect=" . urlencode($redirect_url));
    exit;
}

$id_user = intval($_SESSION['user_id']); // Gunakan ID dari sesi karena pengguna sudah login

// Ambil id_paket dari URL jika ada (untuk auto-select)
$selected_paket = isset($_GET['id_paket']) ? intval($_GET['id_paket']) : null;
...
if (!$user) {
    // Ini seharusnya tidak terjadi jika user_id ada, tapi jika ya, paksa logout
    session_destroy();
...
// ----------------------------------------------------
// 3. CEK APAKAH USER SUDAH MEMBELI PAKET YANG DIPILIH
// ----------------------------------------------------

$sudah_beli = false;
if ($selected_paket) {
    $stmt_cek = mysqli_prepare($conn, "SELECT * FROM pembelian WHERE id_user = ? AND id_paket = ? AND status_pembayaran = 'success'");
    if ($stmt_cek) {
        mysqli_stmt_bind_param($stmt_cek, "ii", $id_user, $selected_paket);
        mysqli_stmt_execute($stmt_cek);
        $result_cek = mysqli_stmt_get_result($stmt_cek);
        $sudah_beli = mysqli_num_rows($result_cek) > 0;
        mysqli_stmt_close($stmt_cek);
    }
}

// Jika sudah beli, redirect ke halaman materi
if ($sudah_beli) {
    echo "<script>
        alert('Anda sudah membeli paket ini! Silakan akses materi.');
        window.location.href='kelaspengguna.php?id_paket=$selected_paket';
    </script>";
    exit;
}
...
// ----------------------------------------------------
// 5. PROSES POST (KONFIRMASI PEMBAYARAN)
// ----------------------------------------------------
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Cek lagi apakah sudah beli sebelum insert
    $stmt_cek = mysqli_prepare($conn, "SELECT * FROM pembayaran WHERE id_user = ? AND id_paket = ?");
    if ($stmt_cek) {
        mysqli_stmt_bind_param($stmt_cek, "ii", $id_user, $id_paket);
        mysqli_stmt_execute($stmt_cek);
        $result_cek = mysqli_stmt_get_result($stmt_cek);
        if (mysqli_num_rows($result_cek) > 0) {
            mysqli_stmt_close($stmt_cek);
            echo "<script>alert('Anda sudah membeli paket ini!'); window.location.href='kelaspengguna.php?id_paket=$id_paket';</script>";
            exit;
        }
        mysqli_stmt_close($stmt_cek);
    }
    ...
    if (!$paket) {
        ...
        // A. Generate Verify Code Baru dan Update User
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            // B. Insert Pembayaran (tanpa status_pembayaran karena tidak ada di tabel)
            ...
            // Binding: ssiii (kode_pembayaran, tanggal, id_user, id_paket, nominal)
            ...
            if (mysqli_stmt_execute($stmt_bayar)) {
                ...
                $error_msg = mysqli_stmt_error($stmt_bayar);
                echo "<script>alert('Error insert pembayaran: " . addslashes($error_msg) . "');</script>";
                mysqli_stmt_close($stmt_bayar);
            }
        } else {
            // Error saat update verify code
...
<div class="payment-container">
    ...
    <p class="text-center text-muted mb-4">Selesaikan pembayaran untuk mendapatkan akses ke kelas</p>
    ...
    <form method="POST" id="paymentForm">
        <div class="paket-section">
            ...
            <label class="paket-card <?= ($selected_paket == $paket['id_paket']) ? 'selected' : '' ?>" onclick="selectPaket(<?= $paket['id_paket'] ?>, <?= $paket['nominal'] ?>)">
                <input type="radio" name="id_paket" value="<?= $paket['id_paket'] ?>" <?= ($selected_paket == $paket['id_paket']) ? 'checked' : '' ?> required>
        ...
        <div class="total-section" id="totalSection" style="<?= $selected_paket ? 'display: block;' : 'display: none;' ?>">
            <div class="total-label">Total Pembayaran:</div>
            <div class="total-price" id="totalPrice">
                <?php 
                if ($selected_paket) {
                    foreach ($pakets as $p) {
                        if ($p['id_paket'] == $selected_paket) {
                            echo 'Rp ' . number_format($p['nominal'], 0, ',', '.');
                            break;
                        }
                    }
                } else {
        ...
        <div class="payment-option" id="qrOption">
            ...
            <img src="./gambar/qr.jpeg" alt="QR Code" class="qr-image">
            <p class="info-text">Scan QR Code untuk melakukan pembayaran</p>
        </div>

        <button type="submit" class="btn-confirm" id="btnConfirm" <?= $selected_paket ? '' : 'disabled' ?>>
            Konfirmasi Pembayaran
        </button>
    </form>

    <p class="text-center text-muted mt-3" style="font-size: 12px;">
        Setelah pembayaran, Anda dapat langsung mengakses materi kelas
    </p>
</div>

<script>
    let selectedPaket = <?= $selected_paket ? 'true' : 'false' ?>;
